...
//! [`schemaforge_python::validate_json`] produces the expected validity for
...
fn fixtures_path() -> std::path::PathBuf {
    // Works from both `cargo test` (cwd = workspace root) and
    // `cargo test -p schemaforge-python` (same cwd under Cargo).
    let candidates = [
...
fn parity_fixtures() {
    ...
    for fx in &fixtures {
        ...
        let result = schemaforge_python::validate_json(&schema_str, &instance_str);
